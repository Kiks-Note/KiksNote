...
function GroupsCreation() {
  ...
  const fetchData = useCallback(async () => {
    ...
    }, [LogToExistingRoomStudent, classStudents, fetchAndSetData, inRoom, lock, logToExistingRoom, navigate, numberOfStudentsInClass, user?.id, user.status, ws]);
  ...
  function shuffle(array) {
    ...
    /*     async function saveGroups() {
            ...
            var groupsKey = Object.keys(columns).filter((key) =>
                key.startsWith("g")
            );
    
            try {
                for (const group of groupsKey) {
                    if (columns[group].items.length === 0) {
                    ...
                    if (file) {
                        dataContent.append("file", file);
                    }
    
                    dataContent.append("po_id", user?.id)
                    dataContent.append("start_date", settings.start_date)
                    dataContent.append("end_date", settings.end_date)
                    dataContent.append("students", columns[group].items.map((student) => student.id))
    
    
                    await axios.post("http://localhost:5050/groupes/exportGroups", dataContent
                    ).then((res) => {
                        if (res.status === 200) {
                            toastSuccess("Les groupes ont bien été définis !");
                        }
                    });
                }
