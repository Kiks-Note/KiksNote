...
function App() {
  ...
  async function saveGroups() {
    ...
    groupsKey.forEach((group) => {
      ...
      formData.append("po_id", user?.id);
      formData.append("start_date", settings.start_date);
      formData.append("end_date", settings.end_date);
      formData.append(
        "students",
        columns[group].items.map((student) => student.id)
      );

      axios.post(`http://localhost:5050/groupes/exportGroups`, formData);
    });
