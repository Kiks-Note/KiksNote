...
import IdeaCard from "../../components/profil/IdeaCard.js";
  ...
  useEffect(() => {
    (async () => {
      const response = await axios.get(
        `${process.env.REACT_APP_SERVER_API}/inventory/ideaByUser/${user?.id}`
      );
      ...
      url(${userProfil.imagebackground})`,
          ...
          >
            ...
            {userProfil && userProfil.status === "etudiant" && (
              <Typography>Classe : {userProfil.class}</Typography>
            )}
            <Typography>
