...
function MostParticipantsChart() {
    ...
    const fetchData = async () => {
        try {
            const response = await axios.get("http://localhost:5050/blog/stats/:id/participant"); // Remplacez l'URL par la route appropriée pour récupérer les blogs
            const blogs = response.data;

            const sortedBlogs = blogs.sort((a, b) => b.participant - a.participant);
            const topBlogs = sortedBlogs.slice(0, 10);

            const labels = topBlogs.map((blog) => blog.name);
